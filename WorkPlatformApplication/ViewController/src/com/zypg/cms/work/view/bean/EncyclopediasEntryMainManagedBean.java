...
import com.zypg.cms.work.model.viewobject.entry.query.CmsEncyclopediasEntryVORowImpl;
...
public class EncyclopediasEntryMainManagedBean extends CustomManagedBean {
    CustomTableModel<CmsEncyclopediasEntryVORowImpl> tableModel =
        new CustomTableModel<CmsEncyclopediasEntryVORowImpl>("WorkAMDataControl", "EntryAM", "CmsEncyclopediasEntryVO",
                                                             20, "findEncyclopediasEntryVC");
    ...
    public EncyclopediasEntryMainManagedBean() {
    }
    ...
    public void tableDblClick(ClientEvent clientEvent) {
        ...
        if (indexStr != null && indexStr.length() > 0 && (indexStr.split(",").length == 1)) {
            ...
            String url =
                request.getContextPath() + "/faces/work/entry/inline/relatedEncyclopediasEntryView.jsf?docId=" + docId;
            this.openWindow(url);
        }
    }
    ...
    public void downloadEntryActionListener(ActionEvent actionEvent) {
        ...
        this.setExpressionValue("#{pageFlowScope.downloadFileName}", sdf.format(new Date()) + "＿百科条目.zip");
        this.executeCompomentAction(this.getBatchDownloadButton().getClientId());
    }

    public void setTableModel(CustomTableModel<CmsEncyclopediasEntryVORowImpl> tableModel) {
        this.tableModel = tableModel;
    }

    public CustomTableModel<CmsEncyclopediasEntryVORowImpl> getTableModel() {
        return tableModel;
    }
