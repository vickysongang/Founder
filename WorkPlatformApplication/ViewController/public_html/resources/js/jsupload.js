...
JsUpload.prototype.init = function (settings) {
    settings = JsUpload.extend( {
        name : '__jsupload__', file : null, validUrl : '', uploadUrl : '', progressUrl : ''
    },
    settings || 
    {
    });
    var name = settings.name, file = settings.file, form = file.form, validUrl = settings.validUrl, uploadUrl = settings.uploadUrl, progressUrl = settings.progressUrl, iframe = JsUpload.iframe(name);
...
JsUpload.prototype.change = function (file, fn) {
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        var form = document.createElement('form'), target = JsUpload.attr(file, '__target__'), action = JsUpload.attr(file, '__valid__'), 
        //临时file对象
        clone, 
        //当前系统时间
        now = (new Date()).getTime(), jsonp = 'jsonp' + now;
...
JsUpload.prototype.upload = function (button, file, fn) {
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        var form = button.form, now = (new Date()).getTime(), jsonp = 'jsonp' + now, action = form.action;
...
JsUpload.progress = function (progressUrl, button, fn) {
    ...
    JsUpload.Ajax.request( {
        url : progressUrl, method : 'get', dataType : 'text', success : function (data) {
...
JsUpload.Ajax = {
    request : function (options) {
        ...
        options = JsUpload.extend( {
            method : 'get', async : true, dataType : 'text'
        },
        options || 
        {
        });
        ...
        if (trans.readyState === 4) {
            ...
            if (typeof s === 'number' && s >= 200 && s < 300) {
                ...
                if (typeof options.error === 'function') {
                    var error = {
                        status : trans.status, statusText : trans.statusText
                    };
